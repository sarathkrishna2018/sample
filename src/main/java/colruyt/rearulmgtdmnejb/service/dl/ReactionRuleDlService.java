...
public class ReactionRuleDlService implements Serializable {
	...
	public List<ReactionRule> findByRuleSetId(long ruleSetId) {
		...
		root.fetch("refSourceTypes");
		root.fetch("refActionTypes");
	...
	public void updateLogicallyDeletedDate(ReactionRule reactionRule) {
		...
		Query query= entityManager.createQuery("update ReactionRule reactionRule set reactionRule.logicallyDeletedDate=?1 where reactionRule.reaRuleId=?2", ReactionRule.class);
		query.setParameter(1, updatedDate);
		query.setParameter(2, reactionRule.getReaRuleId());
		query.executeUpdate();
}

	public List<ReactionRule> findAllByRuleSetId(Long rulesetId) {
		List<ReactionRule> ruleList = Lists.newArrayList();
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<ReactionRule> criteriaQuery = criteriaBuilder.createQuery(ReactionRule.class);
		Root<ReactionRule> root = criteriaQuery.from(ReactionRule.class);
		...
		try {	
			...
			logger.info("No results for finding by rulesetid", exception);
			ruleList = null;
		}
		return ruleList;
	}
	public void logicallyDeleteRules(List<Long> reactionRuleIds,String logonId) {
		Date updatedDate = new Date();
		Query query= entityManager.createQuery("update ReactionRule reactionRule set reactionRule.logicallyDeletedDate=?1,reactionRule.lstUpdateBy=?2 where reactionRule.reaRuleId IN ?3", ReactionRule.class);
		query.setParameter(1, updatedDate);
	...
	public List<XPSRuleBo> findAllLogicallyDeletedRules(Date dateForRulesDelete) {		
		...
		Query query = entityManager.createNativeQuery(SQLQueries.FIND_ALL_LOGICALLY_DELETE_RULE.replaceAll(ReactionRuleMgtConstants.SCHEMA, schemaName));
	...
	public List<XPSRuleBo> findAllExpiredRules(Date dateForRulesDelete) {		
		List<XPSRuleBo> rules = new ArrayList<XPSRuleBo>();
		String schemaName = SQLQueries.getSchemaName(entityManager);
		Query query = entityManager.createNativeQuery(SQLQueries.FIND_ALL_EXPIRED_RULE.replaceAll(ReactionRuleMgtConstants.SCHEMA, schemaName));
		query.setParameter(1, dateForRulesDelete, TemporalType.DATE);
		List<Object[]> results = query.getResultList();
		for(Object[] item : results) {
			Long ruleId = ((BigDecimal)item[0]).longValue();
	...
	public long physicalDeleteRule(XPSRuleBo xpsRuleBo){
		Query query = entityManager.createQuery("Delete from  ReactionRule reactionRule where reactionRule.reaRuleId = (?1)");
